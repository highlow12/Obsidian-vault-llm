...
## 1. 목표와 성공 지표
- 자연어로 볼트 내용을 질문하면 근거가 표시된 답변을 3초 이내(Top-K 검색 + LLM 응답 기준)로 제공한다.
...
- **벡터 스토어**: 로컬 우선(Chroma/FAISS 중 하나를 선택지로), 향후 원격 플러그 가능
- **서비스 구성**:  
...
## 9. 품질/성능/보안 요구
- **성능**: 색인 TPS ≥ 30 chunk/초(로컬 CPU 기준), 질의 응답 총 소요 ≤ 3초(캐시/HF 모델 사용 시)  
