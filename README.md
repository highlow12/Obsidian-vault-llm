# Obsidian Vault LLM – 통합 기능/기술 명세(PRD)

## Obsidian 플러그인 실행

이 저장소는 Obsidian 데스크톱용 플러그인으로 빌드할 수 있습니다.

```bash
npm install
npm run build
```

생성되는 파일:
- `main.js`
- `manifest.json`
- `styles.css`

Obsidian의 플러그인 폴더에 위 파일을 복사하면 플러그인이 로드됩니다.

개발자가 바로 구현을 시작할 수 있도록 기능 명세와 기술 명세를 한 문서에 정리했습니다. Obsidian 볼트의 마크다운 노트를 LLM이 검색·요약·생성할 수 있도록 하는 로컬 우선(Local-first) 도구를 목표로 합니다.

## 증분 인덱싱 기능

### 개요
볼트의 마크다운 파일을 효율적으로 인덱싱하고 검색할 수 있도록 증분 인덱싱 기능이 구현되었습니다.

### 주요 기능

#### 1. 파일 수정 시간 기반 증분 인덱싱
- **파일시스템 mtime 활용**: 각 파일의 수정 시간(mtime)을 저장하여 변경된 파일만 재인덱싱
- **해시 기반 이중 체크**: 파일 내용의 SHA256 해시를 함께 확인하여 정확성 보장
- **자동 스킵**: 변경되지 않은 파일은 자동으로 건너뛰어 성능 최적화

#### 2. 비동기 임베딩 처리
- **논블로킹 처리**: 대용량 볼트도 UI 블로킹 없이 백그라운드에서 처리
- **진행도 추적**: 실시간으로 인덱싱 진행 상황 모니터링
- **취소 가능**: 사용자가 언제든지 인덱싱 작업을 중단 가능

#### 3. 진행도 UI
- **실시간 모달**: 인덱싱 진행 중 모달 팝업 표시
- **상세 정보**: 
  - 전체 파일 수 / 처리된 파일 수
  - 현재 처리 중인 파일명
  - 진행률 바 및 퍼센트
- **상태 표시**: 초기화, 인덱싱 중, 완료, 에러, 취소 상태 구분

### 사용 방법

1. **설정에서 인덱싱 활성화**
   - Obsidian 설정 > OVL 플러그인 > 인덱싱 활성화

2. **전체 인덱싱 시작**
   - 명령 팔레트 (Ctrl/Cmd + P)
   - "OVL: 볼트 인덱싱 시작" 선택
   - 진행도 모달이 표시되며 실시간 진행 상황 확인 가능

3. **자동 증분 인덱싱**
   - 파일 생성/수정/삭제 시 자동으로 증분 인덱싱 실행
   - 변경된 파일만 재처리하여 빠른 업데이트

### 기술 세부사항

#### 벡터 저장소
- **SQLite 기반**: 안정성과 호환성 우선
- **JSON 형식**: 임베딩 벡터를 JSON으로 직렬화하여 저장
- **코사인 유사도**: 검색 시 코사인 유사도 계산
- **향후 계획**: HNSW 또는 FAISS 기반 고성능 벡터 검색 엔진 통합 예정

#### 인덱싱 로직
1. 파일 수정 시간(mtime) 및 내용 해시 확인
2. 기존 메타데이터와 비교
3. 변경사항 없으면 스킵, 있으면 재인덱싱
4. 기존 청크 및 임베딩 삭제
5. 새로운 청크 생성 및 임베딩 저장

### 성능 최적화
- 변경되지 않은 파일 자동 스킵 (해시 및 mtime 이중 체크)
- 배치 처리를 통한 효율적인 파일 처리
- 비동기 처리로 UI 블로킹 방지

## GitHub Copilot 에이전트 한글 출력 설정

GitHub Copilot 에이전트가 작업 중 모든 설명과 메시지를 한국어로 출력하도록 하려면:

1. **`.github/copilot-instructions.md` 파일 생성**: 리포지토리 루트에 `.github` 디렉토리를 만들고 `copilot-instructions.md` 파일을 추가합니다.

2. **한국어 출력 지시사항 추가**: 파일에 다음 내용을 작성합니다:
   ```markdown
   # Copilot Instructions
   
   - 모든 응답, 설명, 커밋 메시지를 한국어로 작성하세요.
   - Write all responses, descriptions, and commit messages in Korean.
   - 코드 작성 시 주석도 한국어로 작성하세요.
   ```

3. **Copilot 대화 시작 시 명시**: Copilot과 대화를 시작할 때 "한국어로 답변해 주세요" 또는 "Please respond in Korean"이라고 요청합니다.

4. **워크스페이스 설정 파일 활용**: `.vscode/settings.json`에 다음을 추가할 수 있습니다:
   ```json
   {
     "github.copilot.advanced": {
       "language": "ko"
     }
   }
   ```

참고: GitHub Copilot의 언어 설정은 프롬프트 기반이므로, 대화 시작 시 명시적으로 한국어 사용을 요청하는 것이 가장 효과적입니다.

## 1. 목표와 성공 지표
- 자연어로 볼트 내용을 질문하면 근거가 표시된 답변을 3초 이내에 제공한다 (Top-K 검색 + LLM 응답, warm 캐시=프로세스/벡터 스토어/LLM 커넥션이 이미 준비된 상태 / cold 스타트=초기 로드 포함, 목표 5초 이하).
- 새/수정된 노트는 10초 이내에 재색인되어 검색 가능해야 한다.
- 모든 데이터는 기본적으로 로컬에 저장되며, 외부 전송은 사용자가 명시적으로 설정한 LLM API로만 이뤄진다.

## 2. 범위
### 포함
- Obsidian 볼트 내 마크다운(.md) 파일 및 메타데이터(frontmatter, 태그, 링크) 색인
- 로컬 백그라운드 워커/CLI를 통한 임베딩 생성 및 벡터 검색
- 대화형 질의응답(챗), 노트 요약, 새 노트 초안 생성
- 답변 내 출처(파일 경로/헤더/문맥 스니펫) 표시
- 간단한 필터(폴더, 태그)와 개인/워크스페이스 설정 관리

### 제외(차후 고려)
- 동시 편집/협업 기능
- 멀티미디어 파일 처리(이미지/음성)
- 클라우드 싱크/호스팅

## 3. 주요 사용자 시나리오
1) 사용자는 `ovl init`으로 볼트 경로와 LLM API 키를 설정한다.  
2) 워커가 볼트를 스캔해 임베딩을 생성하고 로컬 DB/벡터 스토어에 저장한다.  
3) 사용자가 “지난주 회의록 요약해줘”라고 질의하면, 관련 노트의 스니펫과 함께 답변을 받는다.  
4) 사용자가 답변을 바탕으로 “새 노트로 저장”을 선택하면 지정 폴더에 초안(.md)이 생성된다.  
5) 노트를 수정하면 워커가 변경을 감지하고 해당 파일만 재색인한다.

## 4. 기능 명세(Functional Spec)
1. **초기 설정**
   - CLI 명령: `ovl init --vault <path> --provider openai --api-key <key>`  
   - 설정 파일: `~/.ovl/config.yaml` (볼트 경로, LLM 설정, 필터 규칙, 기본 K 값)
2. **인덱싱/동기화**
   - 첫 실행 시 전체 스캔 → 문서 단위 → 헤더/문단 기준 chunking (기본 300~500 토큰)
   - 변경 감지: 파일 생성/수정/삭제 이벤트(watch) → 해당 파일만 재임베딩/삭제
   - 재시도 큐: 임베딩 실패 시 지수 백오프(초기 1초, 배수 2x) 후 재시도, 최대 3회(기본값) 후 스킵 로그
3. **검색/QA**
   - 벡터 검색 Top-K(기본 8) + 필터(폴더, 태그, frontmatter 키/값)
   - 답변에는 근거 리스트(파일명, 헤더, 스니펫, 토큰 오프셋) 포함
   - 옵션: “소스만 보기”(검색 결과만 반환) vs “LLM 답변”
4. **대화 모드**
   - 세션 ID로 컨텍스트 유지, 최근 10턴(기본값)을 요약해 메모리에 압축
   - 인용 마크다운(각주 또는 각 소스 블록) 형태로 출처 삽입
5. **노트 생성/요약**
   - “새 노트로 저장” 시 템플릿 적용(폴더, 파일명 규칙 `YYYY-MM-DD-title.md`)
   - 기존 노트 요약: 헤더별 요약 + 액션 아이템 추출, frontmatter에 `last_summary_at` 기록
6. **설정/보안**
   - API 키는 OS 보안 스토리지 또는 로컬 암호화 파일에 저장
   - 로그/메트릭 최소화(기본 비활성), 사용자가 opt-in 시 익명 통계 전송

## 5. 데이터 흐름
1) **색인 파이프라인**  
   파일 변경 → 파서(frontmatter, 링크, 본문 추출) → 청크 분할 → 임베딩 생성 →  
   메타데이터/임베딩을 SQLite + 벡터 스토어에 저장 → 상태 플래그 업데이트.
2) **질의응답 흐름**  
   사용자 프롬프트 → 파라메터(K, 필터) 확인 → 벡터 검색 → 컨텍스트 패킹 →  
   LLM 호출(system + retrieval prompt) → 답변과 출처 마크다운 반환 → (옵션) 노트로 저장.

## 6. 데이터 모델(요약)
- **Note**: `id`, `path`, `title`, `tags`, `links`, `frontmatter(json)`, `updated_at`, `hash`
- **Chunk**: `id`, `note_id`, `text`, `embedding`, `position`, `token_count`, `section`
- **Conversation**: `session_id`, `turn`, `role`, `content`, `summary_cache`
- **Settings**: `vault_path`, `provider`, `api_key_ref`, `default_k`, `chunk_size`, `filters`

## 7. 저장소/컴포넌트
- **구현 언어**: TypeScript(Node.js) 기준으로 개발
- **파일 시스템**: Obsidian 볼트 원본(.md)
- **메타데이터 DB**: SQLite (로컬 `~/.ovl/meta.db`)
- **벡터 스토어**: 로컬 우선, 향후 원격 플러그 가능  
  - Chroma: 지속성, 멀티프로세스 접근, 디스크/원격 등 백엔드를 교체할 수 있는 플러그블 스토리지가 필요할 때 기본값  
  - FAISS: 메모리 사용을 줄인 단일 프로세스, 최대 성능이 필요할 때
- **서비스 구성**:  
  - `ovl daemon`: 파일 감시 + 색인 워커  
  - `ovl query`: CLI 질의/챗, 또는 로컬 HTTP API(`localhost:11434` 등)  
  - (옵션) Obsidian 플러그인: 로컬 API 연동 UI

## 8. 프롬프트 설계(초안)
- **System Prompt**: “너는 Obsidian 볼트의 전문 리서치 어시스턴트다. 항상 출처를 마크다운으로 인용하고, 모르는 내용은 추측하지 않는다.”
- **Retrieval Prompt 템플릿**:  
  - 입력: 사용자 질문, 검색된 컨텍스트 리스트(파일명/섹션/본문), 지시사항(길이, 톤)  
  - 출력: 마크다운 답변 + 근거 각주 또는 인라인 블록
- **요약 Prompt**: 헤더별 요약, 액션 아이템, 다음 질문 제안.
- **노트 생성 Prompt**: 템플릿 채우기(제목, 요약, 인용).

## 9. 품질/성능/보안 요구
- **성능**: 색인 TPS 목표 20~30 chunk/초(예: M1 Pro 10코어 CPU, CPU-only, 로컬 임베딩 HF all-MiniLM-L6-v2, chunk_size 400 기준의 참조 값이며, 하위 사양에서는 비례 감소 허용·설정으로 조정), 질의 응답 총 소요 ≤ 3초(캐시/HF 모델 사용 시)  
- **신뢰성**: 작업 큐 재시도, 부분 실패 시 로그와 상태 플래그 유지  
- **보안/프라이버시**: 로컬 우선, 외부 전송 시 명시적 동의 및 최소 데이터 전송; API 키 암호화 저장
- **접근성**: CLI는 기본 텍스트 출력, API는 JSON 스키마 제공

## 10. 테스트 전략
- **단위 테스트**: 파서(프론트매터/링크), 청크 로직, 필터 적용, 프롬프트 빌더
- **통합 테스트**: 소형 볼트로 색인→검색→응답 경로 검증, 삭제/수정 이벤트 처리
- **수동 테스트**: Obsidian 플러그인 UI와의 왕복, 응답 출처 정확도 샘플링

## 11. 구현 상태 및 기능 목록

### ✅ 완료된 기능 (v0.1.0)

#### 1. Obsidian 플러그인 통합 ✅
- **플러그인 초기화 및 생명주기 관리**
  - Obsidian 플러그인 API 완전 통합
  - 설정 탭 UI 제공
  - 리본 아이콘 추가 (message-circle)
  
- **커맨드 팔레트 통합**
  - "OVL 대화 창 열기" 커맨드
  - "대화 JSON에서 마크다운 저장" 커맨드

#### 2. 실시간 채팅 인터페이스 ✅
- **ChatView 구현**
  - 사이드바에 통합된 채팅 UI
  - 실시간 메시지 입력 및 AI 응답 표시
  - Ctrl+Enter로 메시지 전송
  - 세션 ID 자동 생성 (`session-TIMESTAMP`)
  - 타임스탬프 표시
  - 대화 저장 버튼
  - 로딩 상태 UI 처리

#### 3. LLM API 통합 ✅
- **다중 제공사 지원**
  - Google Gemini (`@google/genai` 라이브러리 사용)
  - OpenAI (공식 API)
  - Ollama (로컬 LLM)
  - 커스텀 API 엔드포인트
  
- **API 클라이언트 기능**
  - `requestAssistantReply()`: 대화 턴 기반 AI 응답
  - 시스템 프롬프트 지원
  - Bearer 토큰 자동 처리
  - 에러 로깅 및 처리
  - 응답 형식 자동 파싱

#### 4. 설정 관리 ✅
- **설정 UI**
  - LLM 제공사 선택 (드롭다운)
  - API URL 및 키 입력
  - 모델명 설정
  - 시스템 프롬프트 커스터마이징
  - Gemini 모델 목록 자동 로드
  - 기본 저장 폴더 설정

#### 5. 대화 저장 기능 ✅
- **마크다운 변환**
  - JSON 형식 대화를 마크다운으로 변환
  - 역할별 이모지 표시 (👤 사용자, 🤖 어시스턴트)
  - 타임스탬프 포함
  - 파일명 형식: `YYYY-MM-DD-sessionId.md`
  
- **저장 기능**
  - Obsidian 볼트에 저장
  - 자동 폴더 생성
  - 중복 파일명 방지 (숫자 추가)
  - SaveConversationModal UI 제공

#### 6. CLI 도구 (`ovl`) ✅
- **초기화 명령**
  ```bash
  ovl init --vault /path/to/vault --provider openai --api-key YOUR_KEY
  ```
  - 설정 파일 생성: `~/.ovl/config.yaml`
  - API 키 안전 저장: `~/.ovl/api_key.txt` (600 권한)
  - 볼트 경로 자동 감지
  
- **대화 저장 명령**
  ```bash
  ovl save-conversation --session-id my-session --input turns.json --output /path/to/vault
  ```
  - JSON 파일에서 대화 읽기
  - 마크다운으로 변환 및 저장

#### 7. 에러 로깅 시스템 ✅
- **로깅 인프라**
  - Obsidian 플러그인 로그 통합
  - API 에러 추적
  - 사용자 친화적 에러 메시지

#### 8. 대화 주제 분리 AI ✅ (v0.3.0)
- **의미론적 경계 탐지**
  - Gemini Embedding API를 사용한 벡터 변환
  - 코사인 유사도(Cosine Similarity) 계산
  - 슬라이딩 윈도우 기반 유사도 분석
  - 주제 전환 임계값 감지 (기본: 0.75)
  - 급격한 유사도 하락 감지 (0.15 이상)
  
- **키워드 추출**
  - 한국어 텍스트 키워드 자동 추출
  - 불용어 제거 및 빈도 분석
  - 키워드 메타데이터 생성 (`[Keywords: A, B, C]`)
  - 세그먼트 간 공통 키워드 추출
  
- **다중 노트 저장**
  - 대화를 주제별 세그먼트로 자동 분할
  - 각 세그먼트를 개별 마크다운 노트로 저장
  - 노트 간 링크 자동 생성 (공통 키워드 기반)
  - 메인 인덱스 노트 생성 (전체 주제 목록)
  - 키워드 기반 제목 자동 생성
  
- **자동 활성화**
  - 4턴 이상의 대화일 때 자동으로 주제 분리 분석
  - 단일 주제 감지 시 일반 저장 수행
  - 다중 주제 감지 시 자동으로 분리 저장

---

### 📋 대화 저장 사용 예시

#### CLI 사용 (JSON → 마크다운)

**입력 형식 (conversation.json)**
```json
[
  {
    "role": "user",
    "content": "Obsidian에서 태그를 어떻게 사용하나요?",
    "timestamp": "2024-02-15T10:00:00Z"
  },
  {
    "role": "assistant",
    "content": "Obsidian에서 태그는 #태그명 형식으로 사용합니다.",
    "timestamp": "2024-02-15T10:00:05Z"
  }
]
```

**실행**
```bash
ovl save-conversation --session-id obsidian-tags --input conversation.json --output ./
```

**출력 (2024-02-15-obsidian-tags.md)**
```markdown
# 대화 기록 - obsidian-tags

생성일: 2024-02-15T10:00:00.000Z

---

## 👤 사용자
*2024-02-15T10:00:00Z*

Obsidian에서 태그를 어떻게 사용하나요?

## 🤖 어시스턴트
*2024-02-15T10:00:05Z*

Obsidian에서 태그는 #태그명 형식으로 사용합니다.
```

---

### 📋 주제 분리 AI 사용 예시 (v0.3.0)

#### 자동 주제 분리 저장

**시나리오**: 여러 주제가 섞인 긴 대화

```
👤 사용자: 프로젝트 일정을 확인하고 싶어요
🤖 어시스턴트: 프로젝트는 3월 15일까지 완료 예정입니다.

👤 사용자: 그럼 점심 메뉴 추천해줄래요?
🤖 어시스턴트: 오늘은 파스타나 비빔밥이 좋을 것 같습니다.

👤 사용자: 다시 프로젝트로 돌아가서, 마일스톤은?
🤖 어시스턴트: 주요 마일스톤은 다음과 같습니다...
```

**저장 버튼 클릭 시**:
1. AI가 자동으로 3개의 주제를 감지
   - 주제 1: 프로젝트 일정 (키워드: 프로젝트, 일정, 완료)
   - 주제 2: 점심 메뉴 (키워드: 점심, 메뉴, 추천)
   - 주제 3: 마일스톤 (키워드: 프로젝트, 마일스톤)

2. 3개의 개별 노트로 저장:
   - `대화제목 - 1. 프로젝트, 일정, 완료.md`
   - `대화제목 - 2. 점심, 메뉴, 추천.md`
   - `대화제목 - 3. 프로젝트, 마일스톤.md`

3. 메인 인덱스 노트 생성:
   - `대화제목-인덱스.md` (전체 주제 목록 및 링크)

4. 자동 링크 생성:
   - 주제 1과 주제 3은 "프로젝트" 키워드로 연결
   - 각 노트에 연관 주제 섹션 추가

**출력 예시 (주제 1 노트)**:
```markdown
---
segment: 1/3
keywords: [프로젝트, 일정, 완료]
avgSimilarity: 0.892
---

# 주제 1

## 주요 키워드
- 프로젝트
- 일정
- 완료

## 연관 주제
- [[대화제목 - 3. 프로젝트, 마일스톤|주제 3]] (프로젝트)

## 대화 내용

### 👤 사용자
*2024-02-17T...*

프로젝트 일정을 확인하고 싶어요

### 🤖 어시스턴트
*2024-02-17T...*

프로젝트는 3월 15일까지 완료 예정입니다.
```

**기술 상세**:
- **임베딩**: Gemini Embedding API로 각 턴을 벡터화
- **유사도**: 코사인 유사도로 인접 턴 간 관련성 측정
- **임계값**: 유사도 0.75 미만 또는 0.15 이상 급락 시 주제 전환
- **윈도우**: 직전 2턴을 컨텍스트로 사용하여 정확도 향상
- **캐싱**: 벡터 캐싱으로 중복 API 호출 방지

---

## 12. 향후 마일스톤

### ✅ Phase 1: MVP 완성 (v0.2.0 - 완료!)
**목표: 기본적인 볼트 검색 및 질의응답 기능 구현**

- [x] **파일 인덱싱 시스템**
  - [x] 볼트 전체 스캔 및 마크다운 파싱
  - [x] 프론트매터, 태그, 링크 추출
  - [x] 파일 변경 감지 (watch) 및 자동 재색인
  - [x] SQLite 메타데이터 저장
  
- [x] **벡터 검색 구현**
  - [x] 로컬 임베딩 생성 (HuggingFace all-MiniLM-L6-v2)
  - [x] 벡터 스토어 통합 (SQLite 기반)
  - [x] 문서 청킹 (300~500 토큰 단위)
  - [x] Top-K 검색 (기본값: 8)
  
- [x] **RAG 기반 질의응답**
  - [x] 벡터 검색 + LLM 통합
  - [x] 컨텍스트 패킹 및 프롬프트 구성
  - [x] 출처 표시 (파일명, 헤더, 스니펫)
  - [x] "소스만 보기" vs "LLM 답변" 옵션
  
- [ ] **검색 필터** (차후 구현)
  - [ ] 폴더 기반 필터
  - [ ] 태그 필터
  - [ ] Frontmatter 키/값 필터
  - [ ] 날짜 범위 필터

**완료일: 2026-02-17**

### 📖 사용 방법 (v0.2.0)

#### 1. 플러그인 설치
```bash
# 의존성 설치
npm install

# 플러그인 빌드
npm run build

# 생성된 파일을 Obsidian 플러그인 폴더에 복사
# - main.js
# - manifest.json
# - styles.css
```

#### 2. 인덱싱 활성화
1. Obsidian 설정 → 커뮤니티 플러그인 → "Obsidian Vault LLM" 활성화
2. 플러그인 설정에서 "인덱싱 활성화" 토글
3. 설정값 조정 (선택사항):
   - 청크 크기: 기본 400 토큰
   - 청크 오버랩: 기본 50 토큰
   - 검색 결과 수: 기본 8

#### 3. 초기 인덱싱 실행
- 커맨드 팔레트 (Ctrl/Cmd + P) 열기
- "OVL: 볼트 인덱싱 시작" 실행
- 진행 상황은 알림으로 표시됨 (10개 파일마다)

#### 4. RAG 기반 대화 사용
1. 리본에서 메시지 아이콘 클릭 또는 "OVL 대화 창 열기" 커맨드 실행
2. "RAG 사용" 체크박스 활성화
3. 질문 입력 (예: "프로젝트 관련 노트를 요약해줘")
4. 옵션:
   - **RAG 사용**: 볼트에서 관련 문서를 검색하여 컨텍스트로 제공
   - **소스만 보기**: 검색 결과만 표시 (LLM 호출 없음)

#### 5. 자동 재색인
- 파일 생성, 수정, 삭제 시 자동으로 인덱스 업데이트
- 100ms 디바운스로 연속 수정 시 효율적 처리

### ⚠️ 알려진 제한사항 (v0.2.0)

1. **성능**: 대규모 볼트(1000개 이상 파일)에서 검색이 느릴 수 있음
   - 원인: 전체 임베딩을 메모리에 로드하여 선형 검색
   - 개선 예정: ANN 알고리즘 또는 전용 벡터 DB 도입

2. **문장 분할**: 약어(Dr., Mr.)나 소수점에서 잘못 분할될 수 있음
   - 개선 예정: 더 정교한 문장 경계 감지

3. **첫 실행**: 임베딩 모델을 HuggingFace에서 다운로드 (약 50MB)
   - 이후 실행은 로컬 캐시 사용

4. **검색 필터**: 아직 미구현 (폴더, 태그, 날짜 필터 등)


---

### 🚀 Phase 2: 검색 및 필터링 고도화 (다음 목표)
**목표: RAG 검색 정확도와 사용자 경험 향상**

#### 우선순위 1: 검색 필터 구현
- [ ] **폴더 기반 필터**
  - [ ] 특정 폴더만 검색 (예: "Projects/2024" 폴더만)
  - [ ] 폴더 제외 필터 (예: "Archive" 제외)
  - [ ] 설정 UI에서 기본 필터 폴더 지정
  
- [ ] **태그 필터**
  - [ ] 특정 태그가 있는 노트만 검색 (#project, #meeting 등)
  - [ ] 다중 태그 AND/OR 조건
  - [ ] 태그 제외 필터
  
- [ ] **Frontmatter 필터**
  - [ ] 특정 키/값 기준 필터 (status: active)
  - [ ] 날짜 범위 필터 (created, modified)
  - [ ] 커스텀 필드 필터

**예상 소요: 2~3주**

#### 우선순위 2: 성능 최적화
- [ ] **벡터 검색 성능 개선**
  - [ ] ANN(Approximate Nearest Neighbor) 알고리즘 도입
    - FAISS 또는 hnswlib 통합 고려
    - 1000개 이상 파일에서 검색 속도 개선 목표
  - [ ] 임베딩 캐싱 전략
  - [ ] 배치 임베딩 처리 (10개씩 묶어서 처리)
  
- [ ] **메모리 사용 최적화**
  - [ ] 온디맨드 임베딩 로드 (전체 로드 대신)
  - [ ] LRU 캐시 적용
  - [ ] 대용량 파일 처리 개선 (청크 크기 동적 조정)

**예상 소요: 3~4주**

#### 우선순위 3: 사용자 경험 개선
- [ ] **대화 컨텍스트 관리**
  - [ ] 대화 세션 컨텍스트 유지 (현재는 단일 턴만)
  - [ ] 최근 N턴 자동 요약 및 메모리 압축
  - [ ] 대화 히스토리 검색 및 불러오기
  
- [ ] **노트 생성 및 요약**
  - [ ] 기존 노트 요약 (헤더별, 전체)
  - [ ] 액션 아이템 자동 추출
  - [ ] 템플릿 기반 새 노트 생성 강화
  - [ ] Frontmatter 자동 생성 (태그, 날짜, 카테고리 등)
  
- [ ] **Obsidian UI 통합 강화**
  - [ ] 에디터 내 인라인 AI 어시스턴트 (선택 텍스트에 대한 작업)
  - [ ] 우클릭 컨텍스트 메뉴 통합
  - [ ] 노트 링크 자동 제안
  - [ ] 미리보기 패널 통합

**예상 소요: 4~6주**

#### 우선순위 4: 추가 LLM 지원
- [ ] **다중 LLM 모델 지원**
  - [ ] Claude (Anthropic)
  - [ ] Llama (Meta)
  - [ ] 로컬 모델 (llama.cpp, Ollama 확장)
  - [ ] 모델별 프리셋 설정
  - [ ] 작업별 모델 자동 선택 (요약용/대화용/검색용)

**예상 소요: 2~3주**

**Phase 2 총 예상 완료: 3~4개월**

---

### 🌟 Phase 3: 고급 AI 기능 확장 (장기 목표)
**목표: 지능형 노트 관리 및 분석 기능 추가**

#### 우선순위 1: 지능형 노트 연결 및 분석
- [ ] **자동 노트 연결**
  - [ ] 의미적으로 관련된 노트 자동 링크 제안
  - [ ] 노트 간 관계 시각화 (그래프 뷰 통합)
  - [ ] 중복/유사 노트 감지 및 병합 제안
  
- [ ] **고급 검색 기능**
  - [ ] 그래프 기반 관계 검색 (A와 연결된 노트 중 B와 관련된 것 찾기)
  - [ ] 하이브리드 검색 (키워드 + 벡터 조합)
  - [ ] 시간 기반 트렌드 분석 (특정 주제의 시간별 변화)
  - [ ] 질문-답변 체인 (다단계 추론)

**예상 소요: 2~3개월**

#### 우선순위 2: 멀티모달 지원
- [ ] **이미지 처리**
  - [ ] OCR을 통한 이미지 내 텍스트 추출
  - [ ] Vision API를 통한 이미지 설명 생성
  - [ ] 다이어그램 및 차트 분석
  - [ ] 이미지를 검색 컨텍스트에 포함
  
- [ ] **문서 처리**
  - [ ] PDF 파일 파싱 및 인덱싱
  - [ ] 표(Table) 데이터 추출 및 분석
  - [ ] 코드 블록 특화 검색 및 분석

**예상 소요: 2~3개월**

#### 우선순위 3: 플러그인 생태계 통합
- [ ] **Obsidian 플러그인 연동**
  - [ ] 캘린더 플러그인 (일정 기반 노트 검색)
  - [ ] 태스크 플러그인 (TODO 추출 및 우선순위)
  - [ ] Dataview 쿼리 지원 (쿼리 결과를 LLM 컨텍스트로)
  - [ ] Kanban 보드 생성
  
- [ ] **확장 API**
  - [ ] 커스텀 뷰 API
  - [ ] 플러그인 간 통신 프로토콜
  - [ ] 서드파티 통합 SDK

**예상 소요: 2~3개월**

#### 우선순위 4: 다중 볼트 및 협업
- [ ] **다중 볼트 지원**
  - [ ] 프로파일 전환 (개인/업무 볼트 분리)
  - [ ] 볼트 간 검색 (여러 볼트를 동시에 검색)
  - [ ] 통합 설정 관리
  
- [ ] **협업 기능 (초기 탐색)**
  - [ ] 공유 볼트 검색 (읽기 전용)
  - [ ] 팀 프롬프트 템플릿 공유
  - [ ] 협업 세션 기록

**예상 소요: 2~3개월**

**Phase 3 총 예상 완료: 8~12개월**

---

### 🔒 Phase 4: 프로덕션 준비 및 안정화 (지속 개선)
**목표: 보안, 프라이버시, 안정성 강화 및 커뮤니티 성장**

#### 우선순위 1: 보안 및 프라이버시
- [ ] **API 키 보안 강화**
  - [ ] OS Keychain 통합 (macOS Keychain, Windows Credential Manager)
  - [ ] 암호화 저장 강화 (AES-256)
  - [ ] 마스터 패스워드 옵션
  
- [ ] **데이터 프라이버시**
  - [ ] 로컬 전용 모드 (외부 API 호출 완전 차단)
  - [ ] 데이터 익명화 옵션 (외부 전송 시 개인정보 제거)
  - [ ] 감사 로그 (API 호출 기록)
  - [ ] GDPR 준수 체크리스트
  
- [ ] **권한 관리**
  - [ ] 플러그인 권한 세분화
  - [ ] 볼트별 독립적 권한 설정
  - [ ] API 사용량 모니터링 및 제한

**예상 소요: 1~2개월**

#### 우선순위 2: 테스트 및 품질 보증
- [ ] **자동화 테스트**
  - [ ] 단위 테스트 커버리지 > 80% 달성
  - [ ] 통합 테스트 자동화 (CI/CD 파이프라인)
  - [ ] E2E 테스트 (실제 Obsidian 환경)
  - [ ] 성능 벤치마크 자동화
  
- [ ] **성능 모니터링**
  - [ ] 임베딩 생성 속도 추적
  - [ ] 검색 응답 시간 모니터링
  - [ ] 메모리 사용량 프로파일링
  - [ ] 배터리 소모 최적화 (모바일 대비)

**예상 소요: 지속 개선**

#### 우선순위 3: 문서화 및 커뮤니티
- [ ] **사용자 문서**
  - [ ] 한국어/영어 사용자 가이드
  - [ ] 비디오 튜토리얼 제작
  - [ ] FAQ 및 트러블슈팅 가이드
  - [ ] 실사용 예제 및 베스트 프랙티스
  
- [ ] **개발자 문서**
  - [ ] API 레퍼런스 문서
  - [ ] 아키텍처 다이어그램
  - [ ] 기여 가이드라인
  - [ ] 플러그인 확장 가이드
  
- [ ] **커뮤니티 구축**
  - [ ] GitHub Discussions 활성화
  - [ ] Discord/Slack 커뮤니티
  - [ ] 정기 릴리스 노트
  - [ ] 사용자 피드백 수집 및 반영

**예상 소요: 지속 개선**

#### 우선순위 4: 배포 및 유지보수
- [ ] **릴리스 관리**
  - [ ] 자동 버전 관리
  - [ ] 릴리스 노트 자동 생성
  - [ ] 베타 테스트 프로그램
  - [ ] 롤백 전략
  
- [ ] **업데이트 시스템**
  - [ ] 인앱 업데이트 알림
  - [ ] 자동 마이그레이션 스크립트
  - [ ] 하위 호환성 보장
  - [ ] 설정 백업 및 복원

**예상 소요: 지속 개선**

**Phase 4는 지속적인 개선 프로세스로, 각 Phase와 병행하여 진행됩니다.**

---

## 13. 코드베이스 현황 및 기술 스택

### 프로젝트 구조
```
Obsidian-vault-llm/
├── src/
│   ├── main.ts                    # 플러그인 진입점 및 생명주기 관리
│   ├── api.ts                     # LLM API 클라이언트 (Gemini, OpenAI, Ollama)
│   ├── conversation.ts            # 대화 턴 관리
│   ├── conversationStore.ts       # 대화 저장 및 마크다운 변환
│   ├── settings.ts                # 설정 UI 및 관리
│   ├── types.ts                   # 타입 정의 및 프리셋
│   ├── logging.ts                 # 에러 로깅 시스템
│   ├── ovl.ts                     # CLI 도구
│   ├── parseTurns.ts              # JSON 파싱
│   ├── pluginApi.ts               # 플러그인 API 인터페이스
│   ├── vaultWatcher.ts            # 파일 변경 감지 및 자동 재색인
│   ├── indexing/                  # 인덱싱 시스템
│   │   ├── indexer.ts             # 메인 인덱서 (파이프라인 조정)
│   │   ├── parser.ts              # 마크다운 파싱 (frontmatter, 태그, 링크)
│   │   ├── chunker.ts             # 텍스트 청킹 (문장 단위)
│   │   ├── embeddings.ts          # 임베딩 생성 (Gemini, OpenAI, Local)
│   │   ├── vectorStore.ts         # 벡터 저장 및 검색 (SQLite)
│   │   ├── metadataStore.ts       # 메타데이터 저장 (SQLite)
│   │   └── types.ts               # 인덱싱 타입 정의
│   ├── topicSeparation/           # 주제 분리 AI
│   │   ├── topicSeparationEngine.ts  # 주제 분리 엔진 (코사인 유사도)
│   │   ├── embeddingService.ts    # 임베딩 서비스 (Gemini API)
│   │   ├── keywordExtractor.ts    # 키워드 추출 (한국어 지원)
│   │   ├── multiNoteSaver.ts      # 다중 노트 저장 및 링크 생성
│   │   └── types.ts               # 주제 분리 타입 정의
│   ├── views/
│   │   └── chatView.ts            # 채팅 UI 뷰
│   └── modals/
│       └── saveConversationModal.ts  # 대화 저장 모달
├── tests/                         # 단위 테스트
│   ├── chunker.test.ts
│   ├── parser.test.ts
│   ├── embeddingService.test.ts
│   ├── keywordExtractor.test.ts
│   ├── conversation.test.ts
│   ├── ovl-init.test.ts
│   └── ovl-save-conversation.test.ts
├── manifest.json                  # Obsidian 플러그인 매니페스트
├── package.json                   # 의존성 관리
└── tsconfig.json                  # TypeScript 설정
```

### 기술 스택

#### 핵심 기술
- **언어**: TypeScript 5.9+
- **플랫폼**: Obsidian Plugin API 1.12+
- **빌드**: esbuild (빠른 번들링)
- **테스트**: Node.js 내장 테스트 러너

#### 주요 라이브러리
- **@google/generative-ai** (^0.24.1): Gemini API 통합
- **@xenova/transformers** (^2.17.2): 로컬 임베딩 생성 (HuggingFace 모델)
- **better-sqlite3** (^12.6.2): 고성능 SQLite 데이터베이스
- **chokidar** (^5.0.0): 파일 시스템 감시
- **gray-matter** (^4.0.3): Frontmatter 파싱

### 코드 품질 지표

| 항목 | 현재 상태 | 목표 |
|------|----------|------|
| **총 코드 라인 수** | ~2,700 라인 | - |
| **TypeScript 커버리지** | 100% | 유지 |
| **테스트 커버리지** | ~40% | 80% 이상 |
| **빌드 시간** | ~2초 | 5초 이하 |
| **플러그인 번들 크기** | ~1.0MB | 1.5MB 이하 |

### 알려진 기술적 부채 및 개선 필요 사항

#### 높은 우선순위
1. **벡터 검색 성능**
   - 현재: 선형 검색 (O(n))
   - 문제: 1000개 이상 파일에서 느림
   - 해결책: FAISS/hnswlib 도입 (ANN)

2. **임베딩 생성 속도**
   - 현재: 순차 처리
   - 문제: 대용량 볼트 초기 인덱싱이 느림
   - 해결책: 배치 처리 및 병렬화

3. **메모리 사용량**
   - 현재: 전체 임베딩을 메모리에 로드
   - 문제: 대용량 볼트에서 메모리 과다 사용
   - 해결책: 온디맨드 로딩 및 LRU 캐시

#### 중간 우선순위
4. **테스트 커버리지 부족**
   - 현재: 기본 기능만 테스트
   - 목표: 80% 이상 커버리지
   - 필요: 통합 테스트 및 E2E 테스트

5. **에러 처리 개선**
   - 현재: 기본적인 try-catch
   - 필요: 체계적인 에러 분류 및 복구 전략

6. **로깅 시스템 강화**
   - 현재: console.log 및 Obsidian Notice
   - 필요: 구조화된 로깅, 로그 레벨 관리

#### 낮은 우선순위
7. **코드 중복 제거**
   - 일부 유틸리티 함수 중복
   - 리팩토링 필요

8. **타입 안정성 강화**
   - 일부 `any` 타입 사용
   - 더 엄격한 타입 정의 필요

### 성능 벤치마크 (참고)

테스트 환경: M1 Pro, 10코어, 16GB RAM

| 작업 | 성능 | 목표 |
|------|------|------|
| **파일 파싱** | ~50 파일/초 | 유지 |
| **청킹** | ~100 청크/초 | 유지 |
| **로컬 임베딩** | ~20 청크/초 | 30 청크/초 |
| **Gemini 임베딩** | ~50 청크/초 (API 병렬) | 100 청크/초 |
| **벡터 검색** | ~50ms (100 파일) | 유지 |
| **벡터 검색** | ~500ms (1000 파일) | 100ms 이하 |
| **LLM 응답** | ~2-3초 (캐시) | 유지 |

---

## 14. 기여 가이드

### 현재 프로젝트 상태
프로젝트는 **Phase 1 완료** 상태로, MVP 기능이 모두 구현되었습니다. Phase 2 고도화 작업을 시작할 준비가 되었습니다.

### 기여 우선순위

#### 🔥 긴급 (Phase 2)
1. **검색 필터 구현** - 폴더, 태그, frontmatter 필터
2. **성능 최적화** - ANN 알고리즘 도입, 배치 처리
3. **테스트 커버리지 향상** - 80% 목표

#### 🚀 중요 (Phase 2-3)
4. **대화 컨텍스트 관리** - 다중 턴 대화 지원
5. **노트 요약 및 생성** - AI 기반 노트 관리
6. **UI/UX 개선** - 인라인 어시스턴트, 컨텍스트 메뉴

#### 💡 아이디어 환영 (Phase 3+)
7. **멀티모달 지원** - 이미지, PDF 처리
8. **플러그인 생태계** - 다른 Obsidian 플러그인 연동
9. **협업 기능** - 공유 볼트, 팀 템플릿

### 기여 방법
1. **이슈 생성**: 기여하고 싶은 기능이나 버그를 이슈로 등록
2. **논의**: 구현 방법 및 범위를 maintainer와 논의
3. **개발**: Fork 후 feature 브랜치에서 개발
4. **테스트**: 기존 테스트 통과 및 새 테스트 추가
5. **PR 제출**: 상세한 설명과 함께 Pull Request 제출

### 코딩 가이드라인
- TypeScript 엄격 모드 준수
- 한국어 주석 권장 (영어도 가능)
- 기존 코드 스타일 유지
- 모든 public 함수에 JSDoc 추가
- 테스트 코드 필수

---

## 15. 현재 진행 상황 요약

| 영역 | 진행률 | 상태 |
|------|--------|------|
| **기본 인프라** | 100% | ✅ 완료 |
| **플러그인 통합** | 100% | ✅ 완료 |
| **채팅 UI** | 100% | ✅ 완료 |
| **LLM API 통합** | 100% | ✅ 완료 |
| **대화 저장** | 100% | ✅ 완료 |
| **CLI 도구** | 100% | ✅ 완료 |
| **파일 인덱싱** | 100% | ✅ 완료 |
| **벡터 검색** | 100% | ✅ 완료 |
| **RAG QA** | 100% | ✅ 완료 |
| **주제 분리 AI** | 100% | ✅ 완료 (v0.3.0) |
| **검색 필터** | 0% | 🔜 Phase 2 예정 |
| **성능 최적화** | 0% | 🔜 Phase 2 예정 |

**현재 버전: v0.3.0**  
**최신 기능: 대화 주제 분리 AI - 의미론적 경계 탐지 및 자동 다중 노트 저장**

### Phase별 요약

| Phase | 상태 | 예상 완료 |
|-------|------|-----------|
| **Phase 1: MVP** | ✅ 완료 | 2026-02-17 |
| **Phase 2: 고도화** | 🔄 진행 예정 | 3~4개월 |
| **Phase 3: 확장** | 📋 계획 단계 | 8~12개월 |
| **Phase 4: 안정화** | 📋 지속 개선 | 지속 |

---

## 16. 자주 묻는 질문 (FAQ)

### 일반

**Q: 이 플러그인을 사용하려면 유료 API가 필요한가요?**  
A: 네, LLM API 키가 필요합니다. Gemini (무료 할당량 있음), OpenAI, 또는 로컬 Ollama를 사용할 수 있습니다.

**Q: 오프라인에서도 사용할 수 있나요?**  
A: 부분적으로 가능합니다. 로컬 임베딩 모델 사용 시 인덱싱은 오프라인에서 작동하지만, LLM 응답에는 API 연결이 필요합니다. Ollama를 사용하면 완전 오프라인 사용이 가능합니다.

**Q: 내 데이터는 안전한가요?**  
A: 네. 모든 데이터는 로컬에 저장되며, LLM API에는 필요한 컨텍스트만 전송됩니다. API 키는 안전하게 저장됩니다.

### 사용법

**Q: 초기 인덱싱에 얼마나 걸리나요?**  
A: 볼트 크기에 따라 다릅니다. 100개 파일은 약 1~2분, 1000개 파일은 10~20분 정도 소요됩니다.

**Q: 파일을 수정하면 자동으로 재색인되나요?**  
A: 네. 파일 생성, 수정, 삭제 시 자동으로 해당 파일만 재색인됩니다.

**Q: 검색 결과가 정확하지 않아요.**  
A: 다음을 시도해보세요:
1. 더 구체적인 질문 사용
2. 설정에서 검색 결과 수(topK) 증가
3. 청크 크기 조정 (더 크게 또는 작게)

### 문제 해결

**Q: 인덱싱이 실패합니다.**  
A: 다음을 확인하세요:
1. API 키가 올바른지 확인
2. 인덱싱 로그 확인 (콘솔)
3. 메모리 부족인지 확인 (큰 볼트의 경우)

**Q: 검색이 너무 느려요.**  
A: 현재 대용량 볼트(1000개 이상)에서는 성능 이슈가 있습니다. Phase 2에서 ANN 알고리즘 도입으로 개선 예정입니다.

**Q: 주제 분리가 제대로 안 돼요.**  
A: 주제 분리는 4턴 이상의 대화에서만 작동하며, 의미적으로 구분되는 주제가 있어야 합니다. 짧거나 단일 주제 대화는 분리되지 않습니다.

---

## 17. 개발 환경 설정

### 요구사항
- Node.js 16+
- TypeScript 5+
- Obsidian 1.5.0+

### 설치 및 빌드
```bash
# 의존성 설치
npm install

# 플러그인 빌드
npm run build

# 개발 모드 (자동 재빌드)
npm run dev

# CLI 빌드
npm run build:cli

# 테스트 실행
npm test
```

### 플러그인 설치
1. 빌드 후 생성된 파일을 Obsidian 볼트의 플러그인 폴더에 복사:
   - `main.js`
   - `manifest.json`
   - `styles.css`

2. Obsidian 설정 → 커뮤니티 플러그인 → "Obsidian Vault LLM" 활성화

### 디버깅
- Obsidian 개발자 도구 (Ctrl/Cmd + Shift + I)에서 콘솔 로그 확인
- `console.log()` 및 `appendErrorLog()` 활용

---

## 18. 라이선스 및 연락처

### 라이선스
이 프로젝트는 현재 개발 중이며 라이선스가 명시되지 않았습니다. 사용 전 저작권자에게 문의하세요.

### 연락처
- **GitHub**: [highlow12/Obsidian-vault-llm](https://github.com/highlow12/Obsidian-vault-llm)
- **이슈 트래커**: [GitHub Issues](https://github.com/highlow12/Obsidian-vault-llm/issues)

### 기여자
이 프로젝트는 커뮤니티의 기여로 발전합니다. 기여해 주신 모든 분들께 감사드립니다!

---

## 변경 이력

### v0.3.0 (2026-02-17)
- ✨ 주제 분리 AI 추가 (의미론적 경계 탐지)
- ✨ 키워드 기반 자동 노트 제목 생성
- ✨ 노트 간 자동 링크 생성
- ✨ 메인 인덱스 노트 생성

### v0.2.0 (2026-02-16)
- ✨ 파일 인덱싱 시스템 구현
- ✨ 벡터 검색 기능 추가
- ✨ RAG 기반 질의응답 기능 추가
- ✨ 자동 재색인 기능 추가

### v0.1.0 (초기 버전)
- ✨ Obsidian 플러그인 기본 구조
- ✨ LLM API 통합 (Gemini, OpenAI, Ollama)
- ✨ 채팅 UI 구현
- ✨ 대화 저장 기능
- ✨ CLI 도구 (ovl init, ovl save-conversation)

---

이 문서는 개발 진행 상황을 반영하여 지속적으로 업데이트됩니다.
